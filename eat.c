...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/18 16:35:43 by bguzel            #+#    #+#             */
/*   Updated: 2023/06/18 16:47:05 by bguzel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_message(t_philo *philo, char *str)
{
	...
	if (death_check(philo) == 1
		...
		printf("%lums %d %s", get_time()
			- philo->list->begin_time, philo->id + 1, str);
...
int	any_death(t_philo *philo)
{
	...
	{
		pthread_mutex_lock(philo[i].eat_mutex);
		if ((int)(get_time() - philo[i].last_eat_time)
			>= philo->list->time_to_die)
		{
			pthread_mutex_lock(&philo->list->death_mutex);
			philo->list->death = 42;
			pthread_mutex_unlock(&philo->list->death_mutex);
			pthread_mutex_unlock(philo[i].eat_mutex);
			printf("%lums %d %s", get_time()
				- philo->list->begin_time, philo[i].id + 1, "dead\n");
			return (1);
		}
		pthread_mutex_unlock(philo[i].eat_mutex);
		i++;
