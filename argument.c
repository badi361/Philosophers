...
void	get_forks(t_list *list, t_philo *philo)
{
	...
	i = 0;
	list->forks = malloc(sizeof(pthread_mutex_t) * list->number_of_the_philosophers);
	while (i < list->number_of_the_philosophers)
	{
		philo[i].right_fork = &list->forks[i];
		if (i == 0)
			philo[i].left_fork = &list->forks[list->number_of_the_philosophers - 1];
		else
    ...
    pthread_mutex_init(&list->print_mutex, NULL);
}
...
// tv_sec * 1000 = tv_sec normalde saniye cinsinden bir değer döndürür bunu 1000 ile çarparak milisaniye elde ederiz (*1000 milisaniyeye denktir)
// tv_usec / 1000 = tv_useg normalde mikrosaniye cinsinden bir değer döndürür bunu 1000 ile bölerek milisaniye elde ederiz (/1000 milisaniyeye denktir)
// usec işlem başladığından bu yana geçen mikrosaniyeyi bildirir
...
void	get_sleep_and_eat_mutex(t_philo *philo, t_list *list)
{
	...
	i = 0;
    list->all_sleep_mutex = malloc(sizeof(pthread_mutex_t) * list->number_of_the_philosophers);
    list->all_eat_mutex = malloc(sizeof(pthread_mutex_t) * list->number_of_the_philosophers);
	while (i < list->number_of_the_philosophers)
	{
